...
From the above, the distribution of most of the numeric variables presented are approximately normal, with the exception of crime rate, taxpc, density, percentage young male and polpc that stand out as skewed right and may require a log transformation. We now take a look at a scatterplot matrix to examine the bivariate relationships embedded within this dataset.
...
## Comments - If everything else is the same, if the density increaess, won't the tax per capita be the same as we are assuming the wage levels and everything will be the same?
...
ggplot(crime_wage, aes(x=wkly_wage, y=log(crmrte))) + 
  facet_wrap(~sector, scales = "free") + 
  geom_point() + 
  ggtitle("Wages across each sector against Log(Crime Rate) - without wser") + 
  xlab("Weekly Wage") + 
  ylab("Log(Crime Rate)") +
  geom_smooth(method='lm')
```
...
In this view without the wser outlier, the spread of the wage variables is pretty even and does not warrant transformation for our specifications. We typically use log transform on an income variable when the range is large and the data is highly postively  skewed, we don't see that for  the variables. This outlier reflects a county that has a substantially high wage for service workers, and has a lower crime rate. This is likely an area that has been highly gentrified and is predominantly populated my members of in service roles (with fewer individuals who are in the other, lower paying industries, as shown by the wages in other sectors.) We don't believe that removing this outlier is valid, as this county could have another attribute that is worth investigating as a case study of a "successful" community with a low crime rate, and would be of high interest to our political campaign.
...
```{r}
ggplot(crime, aes(x = log(pctymle), y = log(crmrte))) + 
  geom_point() + 
  ggtitle("Log(Crime Rate) against log(Percentage Young Males)") + 
  xlab("log(Percent Young Male)") + 
  ylab("Log(Crime Rate)") + 
  geom_smooth(method='lm')
```

There does appear to be some positive correlation between log(percent young male) and log(crime rate), so we will include this in our model specifications.
...
The second specification, model2 consists of the variables in the first specification plus those we think are likely to affect the crime rate as well as those that have some correlation with the variables. The variables added in this specification are the log transformed police per capita, probability of conviction, probability of arrest and the percentage of minorities.

The third specification contains all the variables in the dataset. To be noted is that we incorporate all the wage variables.

The fourth specification explores if there are any interraction effects between the variables depicting probabiltiy of punishment (arrest, conviction, sentence)
...
It is clear from the results that the second model has the best balance between parsimony and explaining the variation in the outcome variable. Additionally, the interraction terms did not seem to benefit our model in Specifcation 4 as AIC and BIC increased, and adjusted R squared decreased.
...
(2) Random Sampling
We have 89 of the 100 counties in Carolina in our analysis sample population. As of 2016, we have 80 rural counties, we expect that number to be higher in 1980s, so we can assume that we have a random sample.

(3) Multicolinearlity

We know that there isn't perfect multi colinearlity as R would throw an error saying it has encountered singularity. We have as part of our data analysis have identified few variables which are highly correlated and did not include them in the model.
...
From the above correlation matrix we gather that none of the untransformed explanatory variables have a correlation value of more than 0.41. We can assume that we have no multicolinearlity. 
...
Also, we see observation 25 falls outside the Cook's distance line of 0.5 but within a Cook's distiance of 1. So, this could be an influencial outliers that we should be looking at. The main challenge with this observation is that tax per capita is way more than what we see with all the other observations.
